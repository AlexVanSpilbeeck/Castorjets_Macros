...
#ifndef FUNCTION_PREPARECANVAS_H
#define FUNCTION_PREPARECANVAS_H
...
void PrepareCanvas_flat( TCanvas* &can_, TString label){
  int W = 800;
  int H = 450;
...
void SplitCanvas_equally(TCanvas* &can_, TPad* &pad_abs_, TPad* &pad_ratio_, double left_ = 0.20,   double top_ = 0.05, double right_ = 0.05, double bottom_ = 0.25){
  ...
  pad_abs_ = new TPad("Absolute_Values", "Absolute_Values", 0., 0.5,1.,1. - can_->GetTopMargin());
  pad_abs_->SetLeftMargin(0.16);
  pad_abs_->SetRightMargin(0.02);
  pad_abs_->SetBottomMargin(0.);
  pad_abs_->SetTopMargin( 0.05 * 1/0.6 );
  ...
  can_->cd();
  pad_ratio_ = new TPad("Ratios", "Ratios", 0., 0., 1., 0.5);
  pad_ratio_->SetLeftMargin(0.16);
  pad_ratio_->SetRightMargin(0.02);
  pad_ratio_->SetTopMargin(0);
  pad_ratio_->SetBottomMargin( 0.13 * 1./.4 );
