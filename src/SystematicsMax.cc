...
TFile *SystematicsMax::currentStaticTFile_ = new TFile();

SystematicsMax::SystematicsMax(TString inputdir, TObjArray* filelist, bool isData, const char* outputname) {
    
	std::cout << "constructing SystematicsMax class..." << std::endl;
...
SystematicsMax::~SystematicsMax() { }

void SystematicsMax::Loop() {
  ...
  gSystem->Load("../../../RooUnfold-1.1.1/libRooUnfold.so");
	...
	std::cout << " SystematicsMax Loop function is started " << std::endl;
...
//      while((fn = (TObjString*)next()) ) {
		...
		TThread *fileopener;
		fileopener = new TThread("fileopener",(void(*) (void *))&OpenROOTFile,(SystematicsMax*) this);
...
cout << bins_x << "\t" << hCastorJet_Matrix->GetBinCenter( bins_x) << "\tFakes\t" << fake << "\t" << endl;
	...
	*/
	...
	first += part;
	//strcat(temp,part);
//	sprintf(filename,"output_SystematicsMax_%s",first.c_str());
        if( !isData_) { sprintf(filename,"20141114_Output_SystematicsMax_GEN_" + string_gen_radius + "_DET_" + string_det_radius + "_unsortedE_%i_%s", counter_events, first.c_str()); }
        else{ sprintf(filename,"20141114_Output_SystematicsMax_Data_" + string_det_radius + "_Nevents_%i_%s", counter_events, first.c_str()); }
...
void SystematicsMax::AfterLoopCalculations(TString file) {
...
TString SystematicsMax::getOutputFile() {
    return LoopOutputFile_;
}

TString SystematicsMax::getInputDir() {
	return inputdir_;
}

TString SystematicsMax::getCurrentFile() {
	return currentfile_;
}

void SystematicsMax::setCurrentTFile() {
	currentTFile_ = currentStaticTFile_;
}

void* SystematicsMax::OpenROOTFile(SystematicsMax* arg) {
	currentStaticTFile_ = TFile::Open(arg->getInputDir()+arg->getCurrentFile(),"READ");
	return 0;
}

int SystematicsMax::posLeadingGenJet(std::vector<MyGenJet> JetVector,double etacut,double minptcut) {
...
int SystematicsMax::posLeadingTrackJet(std::vector<MyTrackJet> JetVector,double etacut,double minptcut) {
